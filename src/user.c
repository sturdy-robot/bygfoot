...
user_job_offer(User *user) {
    ...
    if (user->counters[COUNT_USER_SUCCESS] <
        ...
        user_event_add(user, EVENT_TYPE_WARNING, -1, -1, NULL,
                       _("The owners of %s are not satisfied with the recent performance of the team. There are rumours they're looking for a new manager."),
                       user->tm->name);
...
user_event_show_next(void) {
    ...
    switch (event->type) {
        ...
        case EVENT_TYPE_OVERDRAW:
            if (event->value1 == 1)
                game_gui_show_warning(
                        _("You have overdrawn your bank account. The team owners give you %d weeks to get above your drawing credit limit."),
                        const_int("int_finance_overdraw_positive"));
            else
                game_gui_show_warning(
                        _("You have overdrawn your bank account once again. Bear in mind that after the fourth time you get fired.\nThe team owners give you %d weeks to get above your drawing credit limit."),
                        const_int("int_finance_overdraw_positive"));
            break;
        ...
        case EVENT_TYPE_TRANSFER_OFFER_CPU:
            game_gui_show_warning(
                    _("Your offer for %s has been accepted. If you still want to buy him, go to the transfer list and left click on the player."),
                    event->value_string);
            break;
        case EVENT_TYPE_TRANSFER_OFFER_REJECTED_BETTER_OFFER:
            ...
            game_gui_show_warning(
                    _("The owners of %s have rejected your offer (%s / %s) for %s. There was a better offer for the player than yours."),
        ...
        case EVENT_TYPE_TRANSFER_OFFER_REJECTED_FEE_WAGE:
            ...
            misc_print_grouped_int(event->value2, buf3);
            game_gui_show_warning(
                    _("The owners of %s have rejected your offer (%s / %s) for %s. Neither the fee nor the wage you offered were acceptable, they say."),
                    ((Team *) event->value_pointer)->name, buf2, buf3, event->value_string);
            break;
        case EVENT_TYPE_TRANSFER_OFFER_REJECTED_FEE:
            ...
            misc_print_grouped_int(event->value2, buf3);
            game_gui_show_warning(
                    _("The owners of %s have rejected your offer (%s / %s) for %s. The team owners weren't satisfied with the fee you offered."),
                    ((Team *) event->value_pointer)->name, buf2, buf3, event->value_string);
            break;
        case EVENT_TYPE_TRANSFER_OFFER_REJECTED_WAGE:
            ...
            game_gui_show_warning(
                    _("%s of %s has rejected your offer (%s / %s). He wasn't satisfied with the wage you offered."),
                    event->value_string,
        ...
        case EVENT_TYPE_TRANSFER_OFFER_REJECTED_STARS:
            /* A player from a team has rejected a transfer offer. */
            game_gui_show_warning(
                    _("%s of %s has rejected your offer because your team has too many star players already. 'A player of my caliber doesn't play second fiddle,' he was quoted."),
                    event->value_string,
        ...
        case EVENT_TYPE_PLAYER_CAREER_STOP:
            sprintf(buf,
                    _("%s's injury was so severe that he can't play football on a professional level anymore. He leaves your team."),
                    player_of_id_team(event->user->tm, event->value1)->name);
        ...
        case EVENT_TYPE_CHARITY:
            game_gui_show_warning(
                    _("The team owners are very satisfied with your financial management. Since the team's been earning a lot of money lately, they decide to donate half of it to charity."));
            break;
    }
...
user_show_sponsor_continue(void) {
    ...
    sprintf(buf,
            _("Your current sponsor is satisfied with your results and would like to renew the contract. Currently they're paying you %d a week."),
            current_user.sponsor.benefit);
