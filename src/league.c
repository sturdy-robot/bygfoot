...
league_new(gboolean new_id, Country *country)
{
    ...
    new.c.id = (new_id) ? league_id_new : -1;
    new.layer = -1;
...
league_cup_get_next_clid(gint clid, gboolean count_inactive)
{
    ...
    if(i != country.leagues->len - 1)
    {
        ...
        return_value = league_cup_get_next_clid(league->c.id, count_inactive);
    }
    else if(country.allcups->len > 0)
        return_value = ((Cup*)g_ptr_array_index(country.allcups, 0))->c.id;
    else
    {
    ...
    if(i != country.allcups->len - 1)
        return_value = ((Cup*)g_ptr_array_index(country.allcups, i + 1))->c.id;
    else
    {
        League *league = g_ptr_array_index(country.leagues, 0);
        if(query_league_active(league) || count_inactive)
...
league_cup_get_previous_clid(gint clid, gboolean count_inactive)
{
    ...
    {   League *league = g_ptr_array_index(country.leagues, i - 1);
        ...
        return_value = league_cup_get_previous_clid(league->c.id, count_inactive);
    }
    else if(country.allcups->len > 0)
        return_value = ((Cup*)g_ptr_array_index(country.allcups, country.allcups->len - 1))->c.id;
    else
    {
        League *league = g_ptr_array_index(country.leagues, country.leagues->len - 1);
        if(query_league_active(league) || count_inactive)
        return_value = league->c.id;
        else
        return_value = league_cup_get_previous_clid(league->c.id, count_inactive);
...
league_remove_team_with_id(League *league, gint id)
{
    ...
    if(((Team*)g_ptr_array_index(league->c.teams, i))->id == id)
    {
        g_ptr_array_remove_index(league->c.teams, i);
        return;
    }
...
league_season_start(League *league)
{
    ...
    gboolean user_champ = 
    (team_is_user(
        g_array_index(league_table(((League*)g_ptr_array_index(country.leagues, 0)))->elements, TableElement, 0).team) != -1);
    gboolean league_above_talent =
    (team_get_average_talents(league->c.teams) > league->average_talent *
    ...
    if(user_champ)
    {
        tm = g_array_index(league_table(((League*)g_ptr_array_index(country.leagues, 0)))->elements, TableElement, 0).team;
...
/*            team_get_average_talents(league->c.teams), league_above_talent); */
    ...
    team_is_top = 
        (team_get_league_rank(team, NULL) <=
...
handle_required_reserve_relegation(const TeamMove *move, GArray *team_movements)
{
    ...
    for (i = 0; i < move->dest_idcs->len; i++) {
        ...
        const League *dest_league = g_ptr_array_index(country.leagues, idx);
        ...
        for (j = 0; j < dest_league->c.teams->len; j++) {
            gint k;
            TeamMove new_move;
                           ...
                           GArray *move_summaries, gint current_layer)
{
    ...
    while (i >= 0) {
        ...
        if(debug > 70) {
            gint dest_idx = g_array_index(move->dest_idcs, gint, 0);
            const League *dest_league = g_ptr_array_index(country->leagues,
                                                          dest_idx);
                                     ...
                                     gboolean up)
{
    ...
    new_move.tm = g_ptr_array_index(prom_games_teams, i);
...
league_get_team_movements(League *league, GArray *team_movements)
{
    ...
    g_ptr_array_sort_with_data(league->c.teams, team_compare_func,
               GINT_TO_POINTER(TEAM_COMPARE_LEAGUE_RANK));
}
...
league_team_movements_destinations(GArray *team_movements, const gint *league_size)
{
    ...
    for(i=0;i<country.leagues->len;i++) {
        League *league = g_ptr_array_index(country.leagues, i);
        league_cur_size[i] = league->c.teams->len;
    }
...
league_add_table(League *league)
{
    ...
    for(i = 0; i < league->c.teams->len; i++)
    {
        new_table_element = 
            table_element_new(
                g_ptr_array_index(league->c.teams, i), i);
...
league_cup_get_teams(gint clid)
{
    ...
    (GPtrArray*)league_from_clid(clid)->c.teams :
    cup_from_clid(clid)->c.teams;
}
                                ...
                                const GArray *team_movements)
{
    ...
    for (i = 0; i < team_movements->len; i++) {
        ...
        for (j = 0; j < move->dest_idcs->len; j++) {
            ...
            const League *dest_league = g_ptr_array_index(country.leagues,
                                                          idx);
            if(dest_league->c.id == league->c.id)
                return FALSE;
        }
    }
...
{
    const League *lowest_league = g_ptr_array_index(country.leagues, country.leagues->len - 1);
    return lowest_league->layer == league->layer;
}
