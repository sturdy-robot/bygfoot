...
fixture_write_league_fixtures(League *league)
{
     ...
     * ordering of the teams list that we had prior to changing league->c.teams
    ...
    for(i = 0; i < max_rr;)
    {
    	fixture_write_round_robin((gpointer)league, -1, misc_copy_ptr_array(teams),
                                  (i == max_rr - 1), -1, league->rr_breaks, i - 1, TRUE);
    	i += (i < max_rr - 1) ? 2 : 1;
    }
    ...
    if(league->joined_leagues->len > 0)
    {
        ...
        if(league->round_robins < max_rr)
            fixture_remove_rrs(league->fixtures, league->c.id, league->c.id, max_rr - league->round_robins);
...
fixture_write_cup_fixtures(Cup *cup)
{
	...
	fixture_write_cup_round_robin(
		...
		g_array_index(cup->rounds, CupRound, 0).team_ptrs), TRUE);
    else
 ...
 *  This fuction assumes that the current values of week and week_round
 *  are the week / week_round that just completed.
...
fixture_update(Cup *cup)
{
	...
	fixture_write_cup_round_robin(cup, round + 1, teams, FALSE);
    else
	fixture_write_knockout_round(cup, round + 1, teams, FALSE);
}
	   ...
	   g_array_index(fixtures, Fixture, i).result[1][0])
	    fixture_write(fixtures, g_array_index(fixtures, Fixture, i).teams[1],
			  ...
			  fixture_get_free_round(week, NULL, g_array_index(fixtures, Fixture, i).teams[0]->id,
						 g_array_index(fixtures, Fixture, i).teams[1]->id, TRUE),
			  &cup->c, round, replay_number,
	...
	g_array_append_val(cupround->tables, table_group[i]);
        ...
        for(j = 0; j < cupround->round_robins;)
        {
            fixture_write_round_robin((gpointer)cup, cup_round, 
                                      ...
                                      cupround->rr_breaks, j - 1, can_sched_current_round);
                          ...
                          gint rr_break_idx, gboolean can_sched_current_round)
{
	...
	fixture_write_round_robin_matchday(fixtures, cup_round, teams, i,
					   week_number, competition, home_advantage,
                                           can_sched_current_round);
    }
	...
	for(i = 0; i < len - 1; i++)
	{
		...
		fixture_write(fixtures,
			      ...
			      competition, cup_round, 0, home_advantage, FALSE, FALSE);
           }
	}
    }
                                   ...
                                   gboolean can_sched_current_round)
{
    ...
    gint week_round_number = fixture_get_free_round(week_number, teams, -1, -1, can_sched_current_round);
	...
	fixture_write(fixtures, home_team, away_team, week_number, 
		      week_round_number, competition, cup_round,
                             ...
                             gboolean can_sched_current_round)
{
    ...
    week_number = league_cup_get_week_with_break(cup->c.id, first_week);
    week_round_number =
	fixture_get_free_round(week_number, teams, -1, -1, can_sched_current_round);
	...
	week_number = (round->two_match_week) ? 
	    week_number :
            league_cup_get_week_with_break(cup->c.id, week_number + cup->week_gap);
...
fixture_write(GArray *fixtures, Team *home_team, Team *away_team, gint week_number,
	      gint week_round_number, Competition *competition,
	      gint cup_round, gint replay_number,
...
gint
fixture_get_free_round(gint week_number, const GPtrArray *teams, gint team_id1, gint team_id2, gboolean is_current_round_free)
{
#ifdef DEBUG
	...
	for(j=0;j<league->fixtures->len;j++)
		...
		g_array_index(league->fixtures, Fixture, j).attendance == -1) &&
	       ((teams == NULL &&
		(query_fixture_team_involved((&g_array_index(league->fixtures, Fixture, j)), team_id1) ||
		 query_fixture_team_involved((&g_array_index(league->fixtures, Fixture, j)), team_id2))) || 
		(teams != NULL &&
		 (query_team_is_in_teams_array(g_array_index(league->fixtures, Fixture, j).teams[0], teams) ||
		  query_team_is_in_teams_array(g_array_index(league->fixtures, Fixture, j).teams[1], teams)))) &&
	       g_array_index(league->fixtures, Fixture, j).week_round_number > max_round)
		max_round = g_array_index(league->fixtures, Fixture, j).week_round_number;
    }
	...
	for(j=0;j<cup->fixtures->len;j++)
		...
		g_array_index(cup->fixtures, Fixture, j).attendance == -1) &&
	       ((teams == NULL &&
		(query_fixture_team_involved((&g_array_index(cup->fixtures, Fixture, j)), team_id1) ||
		 query_fixture_team_involved((&g_array_index(cup->fixtures, Fixture, j)), team_id2))) || 
		(teams != NULL &&
		 (query_team_is_in_teams_array(g_array_index(cup->fixtures, Fixture, j).teams[0], teams) ||
		  query_team_is_in_teams_array(g_array_index(cup->fixtures, Fixture, j).teams[1], teams)))) &&
	       g_array_index(cup->fixtures, Fixture, j).week_round_number > max_round)
		max_round = g_array_index(cup->fixtures, Fixture, j).week_round_number;
    }
    if (!is_current_round_free && week_number == week &&
        max_round + 1 == week_round)
        return max_round + 2;
    return max_round + 1;
}
...
fixture_get_week_list(gint week_number, gint week_round_number)
{
	...
	for(j=0;j<league->fixtures->len;j++)
		...
		g_ptr_array_add(fixtures, &g_array_index(league->fixtures, Fixture, j));
    }
	...
	for(j=0;j<cup->fixtures->len;j++)
		...
		g_ptr_array_add(fixtures, &g_array_index(cup->fixtures, Fixture, j));
    }
...
fixture_get_latest(const Team *tm, gboolean with_cups)
{
	...
	if(league->c.id == tm->league->c.id)
	{
		...
		if(g_array_index(league->fixtures, Fixture, j).attendance == -1)
		    break;
		else if(g_array_index(league->fixtures, Fixture, j).teams[0] == tm ||
			g_array_index(league->fixtures, Fixture, j).teams[1] == tm)
		    g_ptr_array_add(latest, &g_array_index(league->fixtures, Fixture, j));
	}
    }
	...
	for(i=0;i<country.allcups->len;i++)
	{
		...
		if(g_array_index(cup->fixtures, Fixture, j).attendance == -1)
		    break;
		else if(g_array_index(cup->fixtures, Fixture, j).teams[0] == tm ||
			g_array_index(cup->fixtures, Fixture, j).teams[1] == tm)
		    g_ptr_array_add(latest, &g_array_index(cup->fixtures, Fixture, j));
	}	
    }
...
fixture_get_coming(const Team *tm)
{
	...
	if(league->c.id == tm->league->c.id)
	{
		...
		if(g_array_index(league->fixtures, Fixture, j).attendance != -1)
			...
			g_array_index(league->fixtures, Fixture, j).teams[1] == tm)
		    g_ptr_array_add(coming, &g_array_index(league->fixtures, Fixture, j));
	}
    }
	...
	for(j=cup->fixtures->len - 1; j >= 0; j--)
		...
		break;
		    ...
		    g_array_index(cup->fixtures, Fixture, j).teams[1] == tm)
		g_ptr_array_add(coming, &g_array_index(cup->fixtures, Fixture, j));
    }
...
fixture_get_matches(const Team *tm1, const Team *tm2)
{
	...
	if(league->c.id == tm1->league->c.id)
		...
		if(g_array_index(league->fixtures, Fixture, j).attendance == -1)
		    break;
		else if((g_array_index(league->fixtures, Fixture, j).teams[0] == tm1 &&
			 g_array_index(league->fixtures, Fixture, j).teams[1] == tm2) ||
			(g_array_index(league->fixtures, Fixture, j).teams[0] == tm2 &&
			 g_array_index(league->fixtures, Fixture, j).teams[1] == tm1))
		    g_ptr_array_add(matches, &g_array_index(league->fixtures, Fixture, j));
	    }
    }
	...
	for(j=0;j<cup->fixtures->len;j++)
	{
	    if(g_array_index(cup->fixtures, Fixture, j).attendance == -1)
		break;
	    else if((g_array_index(cup->fixtures, Fixture, j).teams[0] == tm1 &&
		     g_array_index(cup->fixtures, Fixture, j).teams[1] == tm2) ||
		    (g_array_index(cup->fixtures, Fixture, j).teams[0] == tm2 &&
		     g_array_index(cup->fixtures, Fixture, j).teams[1] == tm1))
		g_ptr_array_add(matches, &g_array_index(cup->fixtures, Fixture, j));
	}
    }
...
fixture_get_last_week_round(gint week_number)
{
	...
	for(j=0;j<cup->fixtures->len;j++)
		...
		week_round_number = MAX(week_round_number, 
					g_array_index(cup->fixtures, Fixture, j).week_round_number);
    }
...
fixture_get_next_week(gint *week_number, gint *week_round_number)
{
    ...
    *week_number = *week_round_number = 1000;
	...
	if(query_league_active(league))
	{
		...
		(fix->week_number == *week_number && 
		 fix->week_round_number < *week_round_number)) &&
	       (fix->competition->id == current_user.tm->league->c.id ||
    ...
    {
        Cup *cup = g_ptr_array_index(country.allcups, i);
	fix = fixture_get_next(cup->c.id, local_week, local_round);
	if((fix->week_number > local_week ||
...
fixture_get_previous_week(gint *week_number, gint *week_round_number)
{
    ...
    *week_number = *week_round_number = -1;
    ...
    {
        Cup *cup = g_ptr_array_index(country.allcups, i);
	fix = fixture_get_previous(cup->c.id, local_week, local_round);
	if((fix->week_number < local_week ||
	   (fix->week_number == local_week && fix->week_round_number < local_round)) &&
	   (fix->week_number > *week_number ||
	    (fix->week_number == *week_number && fix->week_round_number > *week_round_number)))
...
fixture_get_season_results(void)
{
    ...
    for(i=0;i<country.allcups->len;i++) {
        Cup *cup = g_ptr_array_index(country.allcups, i);
	if(query_team_is_in_cup(current_user.tm, cup))
	{
	    fixtures = cup->fixtures;
...
fixture_get_last_scheduled_week(void)
{
    ...
    for(i=0;i<country.allcups->len;i++) {
        Cup *cup = g_ptr_array_index(country.allcups, i);
        week_nr = MAX(week_nr, g_array_index(cup->fixtures, Fixture, cup->fixtures->len - 1).week_number);
    }
...
fixture_remove_rrs(GArray *fixtures, gint clid1, gint clid2, gint to_remove)
{
    ...
    for(i = 0; i < league1->c.teams->len; i++)
    {
        start = (clid1 == clid2) ? i + 1 : 0;
        for(j = start; j < league2->c.teams->len; j++)
        {
            removed = 0;
            for(k = fixtures->len - 1; k >= 0; k--) {
                const Fixture *fixture = &g_array_index(fixtures, Fixture, k);
                const Team *league1_team = g_ptr_array_index(league1->c.teams, i);
                const Team *league2_team = g_ptr_array_index(league2->c.teams, j);
