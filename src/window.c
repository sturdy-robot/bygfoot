...
window_show_help(gint page) {
    ...
    sprintf(buf,
            "<span %s>Bygfoot Football Manager %s</span>\n(c) %s Győző Both (gyboth@bygfoot.com)\nhttp://bygfoot.sourceforge.net",
            const_app("string_help_window_program_name_attribute"), VERS, YEAR);
...
window_show_startup(Bygfoot *bygfoot) {
    ...
    if (country.sid != NULL)
        ...
        misc_callback_show_team_list(combo_country,
                                     (const gchar *) g_ptr_array_index(country_files, country_files->len - 1));
...
window_show_stadium(void) {
    ...
    gfloat lower_bound = const_int("int_team_stadium_ticket_price") -
                         (1 / const_float("float_team_stadium_price_attendance_factor")) + 1;
...
window_show_alr(void) {
    ...
    gtk_spin_button_set_range(GTK_SPIN_BUTTON(lookup_widget(window.alr, "spinbutton_start_week")),
                              week + 1,
                              MIN(week + current_user.counters[COUNT_USER_LOAN], fixture_get_last_scheduled_week()));
    gtk_spin_button_set_range(GTK_SPIN_BUTTON(lookup_widget(window.alr, "spinbutton_weekly_installment")),
                              0, -current_user.debt *
                                 powf(1 + current_user.debt_interest, const_int("int_finance_payback_weeks")) + 1);
}
